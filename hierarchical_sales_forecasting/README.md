...
The filed claim — target money silently disappearing on zero-metric sibling sets — is disproven with receipts: such sets equal-split, depth-0 equals Σ targets to the penny, `reconcile()` is clean (pinned as tests). What the review found instead is worse: the brand-new-IC carve-out classified any zero-metric leaf as a "new hire" and granted it a full equal share — a rep with zero dc_seats out-earned reps *with* seats, and in `cascade_levels` a zero-metric *team* took half a region (500/500 where the data said 1M/0). Now: `new_ic_rule='none'` turns the carve-out off, `cascade_levels` defaults non-final transitions to `'none'` (teams aren't new hires — breaking, deliberately), `on_zero_metric='equal'|'error'|'fallback'` with a `metric_fallback=[...]` chain replaces the hand-written SQL COALESCE, and every fallback/carve-out engagement lands in `combo_report` (`zero_metric_parents`, `zero_metric_fallbacks`, `carveout_nodes`) — the allocation basis is never invisible again.

### What's New in v0.42.0 — the fast path with the traps removed ([#64](https://github.com/shreyasrkarwa/Analytics/issues/64))

First, receipts: the filed wall-clocks don't reproduce on HEAD (107K rows: `enforce_identities` ~5s vs the reported 57 min; `route_targets` 0.03s vs 25 min; scaling verified linear), and "row order changed my results" is disproven — six shuffles × every mode, 0.0 delta, now pinned as tests. The real footgun the filer likely hit: `pd.concat` drops `df.attrs`, so a hand-rolled slice-and-stitch loses the `cascade_row_keys` stamp and downstream key *inference* can genuinely change grouping. So v0.42.0 ships the fast path with that trap structurally removed: `apply_pins(..., subset=['MM_AMER'])` / `enforce_identities(..., subset=[...])` confine the run to the seeds' subtrees — the library computes the closure (each pin's absorption domain pulled in mechanically, escaping pins raise), stitches back preserving index, order and attrs, and the output is proven bit-identical to the full-frame run. Plus across-the-board key-build and pin-lookup vectorization.
...
Pinning money onto a zeroed slice (a no-dc-seats rep's Migration row) used to set `base = cascaded` — the row had no ratio to preserve, the code silently used 1.0, and `anchor='leaves'` then rebuilt the team from the poisoned base: an error three levels from its cause. Now zero-baseline rows *derive* their ratio — from `apply_pins(..., hedge=)` when given (float / `{depth: cum}` / `HedgeByDepth`, resolved like `reconcile`), else from same-parent siblings, else same-depth rows — with one warning when such a row actually receives money. The fix sits at the single choke point under `apply_pins` / `enforce_identities` / `resplit_by_metric`, so every helper inherits it. And #62's claim that grand-total pins re-partition a node to a stale baseline shape: disproven with receipts — allocation is `row × (total / current_total)`, the uniform mix-preserving multiplier, and always was; the observed drift is overlapping-pin composition (#63, next). Now pinned as a regression test.
