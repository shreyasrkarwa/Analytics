    ...
    def generate_synthetic_sellers(self, num_sellers: int) -> pd.DataFrame:
        ...
        for _ in range(num_sellers):
            data.append({
                ...
                # Seniority: 1 = Senior (top performer), 2 = Mid, 3 = Junior
                # Aligned with Territory_Tier encoding (1 = Highest TAM tier)
                # so that abs(Seniority - Territory_Tier) = 0 for ideal matches
                'Seniority': np.random.choice([1, 2, 3], p=[0.2, 0.5, 0.3]),
